...
В данном разделе приводится пример иерархии вложенности по п.~2.6 ГОСТ~19.106 \cite{gost_19_106_Требования_к_программным_документам_выполненным_печатным_способом}.
...
{\tabletextsize
	\begin{longtable}[c]{| >{\centering}m{25mm} | >{\raggedright}m{50mm} | >{\raggedright}m{50mm} }
		\caption{\normalsize Структура входного пакета комплекса\hspace{25cm}} % \hspace{25cm} - хак, для того чтобы было выровнено по левому краю, иначе без головной боли не получилось
		\label{t:tab007} \\		
		\hhline{|-|-|} % горизонтальная линия
		Номер\\слова & \centering{Наименование информации} & \tabularnewline
		\hhline{|=|=|} % горизонтальная двойная линия		
		\endfirsthead % Конец заголовка на 1 странице
		\multicolumn{2}{l}{Продолжение таблицы \thetable} \\ % 6 - число колонок, по-другому не получается
		\hhline{|-|-|}
		% Простой способ
		...
		\endhead
		\hhline{|-|-|}
%		\multicolumn{2}{r}{\tabletextsize см. далее}
		\endfoot
		\hhline{|-|-|}
		\endlastfoot	
		%------------------- табличные данные ------------------
